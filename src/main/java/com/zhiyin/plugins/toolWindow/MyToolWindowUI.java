...
//            autoNavigationAfterSearchAction();
            ...
            ReadAction.nonBlocking(() -> {
                navigateToControllerForCustomRender(project, url);
                return null; // 需要返回点东西给 nonBlocking
...
//            autoNavigationAfterSearchAction();
    ...
    private void navigateToSelectedMethod(Project project, String methodSignature) {
        ReadAction.nonBlocking(() -> {
                    // ---- 1️⃣ 提取 className 和 methodName ----
                    int hashIndex = methodSignature.lastIndexOf('#');
                    int parenIndex = methodSignature.lastIndexOf('(');
                    if (hashIndex == -1 || parenIndex == -1) return null;

                    String className = methodSignature.substring(0, hashIndex);
                    String methodName = methodSignature.substring(hashIndex + 1, parenIndex);

                    JavaPsiFacade psiFacade = JavaPsiFacade.getInstance(project);

                    // ---- 2️⃣ 遍历模块查找方法 ----
                    for (Module module : ModuleManager.getInstance(project).getModules()) {
                        GlobalSearchScope scope = GlobalSearchScope.moduleScope(module).union(GlobalSearchScope.moduleWithDependenciesAndLibrariesScope(module));

                        PsiClass psiClass = psiFacade.findClass(className, scope);
                        if (psiClass != null) {
                            for (PsiMethod method : psiClass.getMethods()) {
                                if (method.getName().equals(methodName)) {
                                    return method; // 找到方法，返回给 UI 线程
                                }
                            }
                        }
                    }
                    return null;
                }).inSmartMode(project) // 等索引可用
                  .expireWith(PluginDisposable.getInstance(project))  // 项目关闭时取消
                  .finishOnUiThread(ModalityState.defaultModalityState(), method -> {
                    // ---- 3️⃣ 回到 EDT 导航 ----
                    if (method != null) {
                        NavigationUtil.activateFileWithPsiElement(method);
                    } else {
                        MyPluginMessages.showWarning("Method not found", "Method not found: " + methodSignature, project);
                    }
                }).submit(AppExecutorUtil.getAppExecutorService());
    }
