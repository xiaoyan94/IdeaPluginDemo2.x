...
public class MyXMLFoldingBuilder extends FoldingBuilderEx {
    ...
    public FoldingDescriptor @NotNull [] buildFoldRegions(@NotNull PsiElement root, @NotNull Document document, boolean quick) {
        ...
        boolean isInLibrary = ProjectFileIndex.getInstance(project).isInLibrary(root.getContainingFile().getVirtualFile());
        if (isInLibrary) {
            module = null;
        }
        ...
        root.accept(new XmlRecursiveElementWalkingVisitor(true) {
            ...
            public void visitXmlTag(XmlTag tag) {
                ...
                handleXmlTagI18nAttribute(tag, project, finalModule, "Title", "value", descriptors, 1);
                handleXmlTagI18nAttribute(tag, project, finalModule, "Field", "label", descriptors, 1);
//                handleXmlTagI18nAttribute(tag, project, finalModule, "Enum", "display", descriptors);
                handleXmlTagI18nAttribute(tag, project, finalModule, "column", "i18n", descriptors, 2);
            }
        });
    ...
    private static void handleXmlTagI18nAttribute(XmlTag tag, Project project, Module module, @NotNull String tagName, @NotNull String attributeName, List<FoldingDescriptor> descriptors, int i18nType) {
        if (tag.getName().equals(tagName)) {
            ...
            String placeholderTextNew = key;

            if (i18nType == 1) {
                placeholderTextNew = MyPropertiesUtil.findModuleDataGridI18nPropertyValue(project, module, key);
            } else if (i18nType == 2) {
                placeholderTextNew = MyPropertiesUtil.findModuleI18nPropertyValue(project, module, key);
            } else if (i18nType == 3) {
                placeholderTextNew = MyPropertiesUtil.findModuleWebI18nPropertyValue(project, module, key);
            }

            if (placeholderTextNew != null) {
                String placeholderText = "\"" + placeholderTextNew + "\"";
