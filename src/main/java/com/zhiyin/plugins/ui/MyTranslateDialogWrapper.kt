...
*//**
 * UI DSL v2 implementation of the translate dialog.
 *
 * Note:
 *  - Compatible with IntelliJ Platform versions that include UI DSL v2 (2023.1+ recommended).
 *  - External callers may inject:
 *      - checkI18nKeyExistsFun (Predicate<String>)
 *      - getPropertiesByValueFun (Function<String, List<Property>>)
 *//*
class MyTranslateDialogWrapper(private val project: Project?, private val module: Module?) : DialogWrapper(project) {
    ...
    init {
        title = "创建资源串 ${module?.name ?: project?.name} - ${module?.let { MyPropertiesUtil.getSimpleModuleName(it) } ?: ""}"
    ...
    // external injection points (default safe no-op)
    ...
    private var getPropertiesByValueFun: Function<String, List<Property>> = Function { emptyList() }
    fun setGetI18nPropertiesFun(getI18nPropertiesFun: Function<String, List<Property>>) {
        this.getPropertiesByValueFun = getI18nPropertiesFun
    }

    // UI controls (we store cells to update easily)
    private lateinit var i18nKeyCell: Cell<JBTextField>
    private lateinit var sourceCHSCell: Cell<JBTextField>
    private lateinit var chineseUnicodeCell: Cell<JBTextField>
    private lateinit var englishCell: Cell<JBTextField>
    private lateinit var englishUnicodeCell: Cell<JBTextField>
    private lateinit var chineseTWCell: Cell<JBTextField>
    private lateinit var chineseTWUnicodeCell: Cell<JBTextField>
    private lateinit var vietnameseCell: Cell<JBTextField>
    private lateinit var vietnameseUnicodeCell: Cell<JBTextField>
    private lateinit var translateBtnCell: Cell<JButton>

    private val simpleModuleName: String? = module?.let { MyPropertiesUtil.getSimpleModuleName(it) }

    override fun createCenterPanel(): JComponent {
        // UI DSL v2 panel builder
        return panel {
            // 原文
            group("原文:") {
                row("简体:") {
                    sourceCHSCell = textField()
                        .columns(40)
                        .focused()
                        .validationOnApply {
                            if (it.text.isEmpty()) error("Please input source text") else null
                        }
                        .validationOnInput {
                            // UI updates on EDT
                            SwingUtilities.invokeLater {
                                val hasText = it.text.isNotEmpty()
                                translateBtnCell.component.isEnabled = hasText
                                chineseUnicodeCell.component.text = it.text.toUnicode()
                                // clear translations & flags when source changed
                                englishCell.component.text = ""
                                englishUnicodeCell.component.text = ""
                                chineseTWCell.component.text = ""
                                vietnameseCell.component.text = ""
                                sourceCHSCell.comment?.text = "Enter to translate/按回车键翻译"
                                i18nKeyCell.component.text = ""
                                inputModel.propertyKeyExists = false
                            }
                            null
                        }
                        // .bindText(inputModel::chinese)
                        .bindText({ inputModel.chinese }, { v -> inputModel.chinese = v })

                    translateBtnCell = button(getTranslateButtonText()) { doTranslateAction() }
                        .enabled(false)

                    // provider switch actions
                    actionsButton(object : DumbAwareAction("有道翻译") {
                        override fun actionPerformed(e: AnActionEvent) = updateApiProviderAndButtonText("YouDao")
                    }, object : DumbAwareAction("微软翻译") {
                        override fun actionPerformed(e: AnActionEvent) = updateApiProviderAndButtonText("Microsoft")
                    }, object : DumbAwareAction("百度翻译") {
                        override fun actionPerformed(e: AnActionEvent) = updateApiProviderAndButtonText("Baidu")
                    })

                    // clear cache action
                    actionButton(object : DumbAwareAction("清空缓存", "Tips:清空翻译缓存", AllIcons.Actions.GC) {
                    ...
                    // add action listener to actual component after creation
                }
            }
            ...
            group("翻译结果:") {
                row("简体Unicode:") {
                    chineseUnicodeCell = textField()
                        .columns(40)
                        .enabled(false)
                        .bindText({ inputModel.chineseUnicode }, { v -> inputModel.chineseUnicode = v })
                }

                row("英文:") {
                    englishCell = textField()
                        .columns(40)
                        .validationOnInput {
                            // update english Unicode and default key if key not exists
                            SwingUtilities.invokeLater {
                                if (it.text.isEmpty() && it.isEnabled) {
                                    error("请输入英文")
                                } else {
                                    englishUnicodeCell.component.text = it.text.toUnicode()
                                    if (!inputModel.propertyKeyExists) {
                                        val key = "com.zhiyin.mes.app.${simpleModuleName ?: "module"}.${it.text.toSnakeCase()}"
                                        i18nKeyCell.component.text = key
                                    }
                                }
                            }
                            null
                        }
                        .bindText({ inputModel.english }, { v -> inputModel.english = v })
                    rowComment("") // placeholder for alignment
                }

                row("英文Unicode") {
                    englishUnicodeCell = textField()
                        .columns(40)
                        .enabled(false)
                        .bindText({ inputModel.englishUnicode }, { v -> inputModel.englishUnicode = v })
                }

                row("繁体:") {
                    chineseTWCell = textField()
                        .columns(40)
                        .bindText({ inputModel.chineseTW }, { v -> inputModel.chineseTW = v })
                }

                row("繁体Unicode") {
                    chineseTWUnicodeCell = textField()
                        .columns(40)
                        .enabled(false)
                        .bindText({ inputModel.chineseTWUnicode }, { v -> inputModel.chineseTWUnicode = v })
                }

                row("越南语:") {
                    vietnameseCell = textField()
                        .columns(40)
                        .bindText({ inputModel.vietnamese }, { v -> inputModel.vietnamese = v })
                }

                row("越南语Unicode") {
                    vietnameseUnicodeCell = textField()
                        .columns(40)
                        .enabled(false)
                        .bindText({ inputModel.vietnameseUnicode }, { v -> inputModel.vietnameseUnicode = v })
                }

                row("I18n key:") {
                    i18nKeyCell = textField()
                        .text("")
                        .columns(40)
                        .validationOnInput {
                            i18nKeyCell.comment?.text = "Length: ${it.text.length}"
                            null
                        }
                        .validationOnApply {
                            if (it.text.isEmpty()) {
                                error("Property key is required")
                            } else if (isI18nKeyExists(it.text) && !inputModel.propertyKeyExists) {
                                i18nKeyCell.comment?.text = "已有key=${it.text}"
                                error("该 property key 和已有key冲突，不能重复")
                            } else {
                                null
                            }
                        }
                        .bindText({ inputModel.propertyKey }, { v -> inputModel.propertyKey = v })
                }
            }

            // after UI built: add enter key behavior
            // We cannot refer to components before they are created, so attach listeners in afterValidate block:
            applyToComponent {
                // add enter press trigger to source field component
                sourceCHSCell.component.addActionListener {
                    translateBtnCell.component.doClick()
                }
            }
        }
    }

    private fun applyToComponent(action: () -> Unit) {
        // helper: run on EDT after UI creation
        invokeLater { action() }
    }
    ...
    private fun updateApiProviderAndButtonText(apiProvider: String) {
        ...
        // update button text on EDT
        invokeLater {
            translateBtnCell.component.text = getTranslateButtonText()
            translateBtnCell.component.doClick()
        }
    }
    ...
    // 翻译按钮点击逻辑（同步入口）
    private fun doTranslateAction() {
        // small read operation: check existing properties by value
        val source = sourceCHSCell.component.text
        // run quick read action to call the injected function safely
        runReadAction {
            val foundProperties = getPropertiesByValueFun.apply(source)
            if (foundProperties.isNotEmpty() && foundProperties.size >= 2) {
                ...
                invokeLater {
                    ...
                    if (foundProperties.size == 2) {
                        englishCell.component.text = foundProperties[1].value?.let { if (!isNative2Ascii) it.unicodeToString() else it } ?: ""
                    }
                    if (foundProperties.size >= 3) {
                        chineseTWCell.component.text = foundProperties[1].value?.let { if (!isNative2Ascii) it.unicodeToString() else it } ?: ""
                        englishCell.component.text = foundProperties[2].value?.let { if (!isNative2Ascii) it.unicodeToString() else it } ?: ""
                    }
                    if (foundProperties.size >= 4) {
                        vietnameseCell.component.text = foundProperties[3].value?.let { if (!isNative2Ascii) it.unicodeToString() else it } ?: ""
                    }

                    translateBtnCell.component.isEnabled = true
                    sourceCHSCell.comment?.text = "值已存在，key=$key"

                    if (TranslateSettingsComponent.getInstance().state.doOKActionWhenI18nKeyExists) {
                        // 自动确认（受配置控制）
                        doOKAction()
                    } else {
                        // focus OK so user can accept
                        getButton(okAction)?.requestFocus()
                    }
                }
            } else {
                // 不存在 key，则发起翻译（网络/耗时）——在新线程执行
                invokeLater {
                    translateBtnCell.component.text = "翻译中..."
                    translateBtnCell.component.isEnabled = false
                    sourceCHSCell.component.isEditable = false
                    sourceCHSCell.comment?.text = ""
                }
                Thread {
    ...
    // 执行翻译（耗时），并在 EDT 更新 UI
    private fun translateAndUpdateUI() {
        val chinese = sourceCHSCell.component.text
        ...
        invokeLater {
            if (chinese.isNotEmpty()) {
                chineseUnicodeCell.component.text = chinese.toUnicode()
            }
            englishCell.component.text = english
            if (english.isNotEmpty()) {
                englishUnicodeCell.component.text = english.toUnicode()
                i18nKeyCell.component.text = "com.zhiyin.mes.app.${simpleModuleName ?: "module"}.${english.toSnakeCase()}"
            }
            chineseTWCell.component.text = cht
            if (cht.isNotEmpty()) chineseTWUnicodeCell.component.text = cht.toUnicode()
            vietnameseCell.component.text = vi
            if (vi.isNotEmpty()) vietnameseUnicodeCell.component.text = vi.toUnicode()

            translateBtnCell.component.text = getTranslateButtonText()
            translateBtnCell.component.isEnabled = true
            sourceCHSCell.component.isEditable = true
            sourceCHSCell.comment?.text = "Enter to input the key"

            invokeLater {
                getButton(okAction)?.requestFocus()
                runWriteAction {
                    CopyPasteManager.getInstance().setContents(StringSelection(i18nKeyCell.component.text))
                }
            }
        }
    }
    ...
    override fun doOKAction() {
        super.doOKAction()
        if (inputModel.propertyKeyExists) {
            MyPluginMessages.showInfo("已复用key", "${i18nKeyCell.component.text}=${sourceCHSCell.component.text}", project)
        } else {
            MyPluginMessages.showInfo("已翻译", "value=${sourceCHSCell.component.text}, key=${i18nKeyCell.component.text}", project)
        }
    }

    // helper: expose click for tests
    fun clickTranslateButton() {
        translateBtnCell.component.doClick()
    }

    fun setSourceCHSText(source: String) {
        sourceCHSCell.component.text = source
    }
}*/
            ...
            group("原文:") {
                row("简体:") {
                    ...
                    val action = object : DumbAwareAction("清空缓存", "清空翻译缓存", AllIcons.Actions.GC) {
                        override fun actionPerformed(e: AnActionEvent) {
                            ClearTranslationCacheAction.doAction()
                            sourceCHSTextField.comment?.text = "缓存已清空"
                            val chinese = sourceCHSTextField.component.text
                            sourceCHSTextField.component.text = ""
                            sourceCHSTextField.component.text = chinese
                        }
                    }
                    actionButton(action)
                }
...
//                }
                ...
                row("繁体:") {
                    chineseTWTextField = textField().columns(COLUMNS_LARGE).enabled(true).validationOnInput {
                ...
                row("繁体Unicode") {
                    chineseTWUnicodeTextField = textField().columns(COLUMNS_LARGE).enabled(false).validationOnApply {
                        if (it.text.isEmpty() && it.isEnabled && !inputModel.propertyKeyExists) {
                ...
                row("越南语:") {
                    vietnameseTextField = textField().columns(COLUMNS_LARGE).enabled(true).validationOnInput {
                        if (it.text.isEmpty() && it.isEnabled && !inputModel.propertyKeyExists) {
                            error("请输入越南语")
                        } else {
                ...
                row("越南语Unicode") {
                    vietnameseUnicodeTextField = textField().columns(COLUMNS_LARGE).enabled(false).validationOnApply {
                        if (it.text.isEmpty() && it.isEnabled && !inputModel.propertyKeyExists) {
...
//            val foundProperties = getPropertiesByValueFun.apply(inputModel.chinese)
            ...
            // val foundProperties = getPropertiesByValueFun.apply(sourceCHSTextField.component.text) // 耗时
