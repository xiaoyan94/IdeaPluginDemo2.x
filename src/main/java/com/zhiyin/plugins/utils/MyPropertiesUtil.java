...
//    private static final Logger LOG = Logger.getInstance(MyPropertiesUtil.class);
    ...
    public static String findModuleI18nPropertyValue(Project project, Module module, String key) {
        return findModuleWebI18nPropertyValue(project, module, key);
    }
    ...
    public static List<Property> findModuleI18nProperties(Project project, Module module, String key) {
        ...
        // 过滤 i18n 目录下的.properties 文件，排除 datagrid 子目录
        properties = properties.stream()
                               .filter(virtualFile -> {
                                   String path = virtualFile.getPath().replace('\\', '/');
                                   return !virtualFiles.contains(virtualFile) &&
                                           path.contains("/i18n/") &&
                                           !path.contains("/datagrid/") &&
        ...
        properties = properties.stream()
                               .filter(virtualFile -> {
                                   ...
                                   return !virtualFiles.contains(virtualFile) &&
                                           path.contains("/i18n/") &&
                                           path.contains("/datagrid/") &&
                                           (path.endsWith(Constants.I18N_DATAGRID_ZH_CN_SUFFIX) ||
                                                   path.endsWith(Constants.I18N_DATAGRID_ZH_TW_SUFFIX) ||
                                                   path.endsWith(Constants.I18N_DATAGRID_EN_US_SUFFIX) ||
                                                   path.endsWith(Constants.I18N_DATAGRID_VI_VN_SUFFIX));
                               })
                               .collect(Collectors.toList());
        virtualFiles.addAll(properties);
    ...
    public static void addPropertyByFileNames(Project project, Module module, String[] propertiesFileName, String key, String value, boolean updateIfExist) {
        ...
        for (String nFileSuffix : propertiesFileName) {
            ...
            if (virtualFiles.isEmpty()) {
                ...
                boolean containsDataGrid = nFileSuffix.contains("datagrid");

                // 过滤出文件名以指定后缀结尾的文件
                List<VirtualFile> matchedFiles = allProperties.stream()
                                                              .filter(vf -> vf.getName().endsWith(nFileSuffix))
                                                              .filter(vf -> {
                                                                  // 根据containsDataGrid：如果true则只要匹配datagrid文件；否则只要匹配非datagrid文件
                                                                  return containsDataGrid == vf.getName().contains("datagrid");
                                                              })
                                                              .collect(Collectors.toList());

                virtualFiles.addAll(matchedFiles);
            }
        }
...
//        PropertiesComponent.getInstance();
        if (i18nFileSuffix.startsWith("web")) {
            ...
            addPropertyByFileNames(project, module, new String[]{i18nFileSuffix}, key, value,
                                   updateIfExist
            );
        }
    }
    ...
    public static List<Property> findModuleI18nPropertiesByValue(Project project, Module module, String value) {
        ...
        ReadAction.run(() -> {
            ...
            // 兜底
            // 非传统 Java Web 项目，使用路径匹配方式
            Collection<VirtualFile> propertiesList = FileTypeIndex.getFiles(
            ...
            propertiesList = propertiesList.stream()
                                   ...
                                   .collect(Collectors.toList());
            virtualFiles.addAll(propertiesList);
...
//        return getKeysFromProperties(value, allProperties, result);
    ...
    public static List<IProperty> getModuleDataGridI18nPropertiesCN(Project project, Module module) {
        ...
        Collection<VirtualFile> properties = FileTypeIndex.getFiles(
                FileTypeManager.getInstance().getFileTypeByExtension("properties"), scope);
        // 过滤 i18n 目录下的.properties 文件，只查 datagrid 子目录
        properties = properties.stream()
                               .filter(virtualFile -> {
                                   String path = virtualFile.getPath().replace('\\', '/');
                                   return !virtualFiles.contains(virtualFile) &&
